...
#| eval = FALSE
# Generic structure
#Must be a df or tibble 
# Declare data and aesthetic
ggplot(data = ,
       mapping = aes()) +
  geom_*() + #short for geometric shape geom_bar = bar graph, histogram, box plot, etc
  scale_*() + #optional: change the behavior of any aesthetic here
  labs() +  #optional: Labels like title, subtitle, axis titles, legend, captions
  facet_*() + #optional: creating smaller plots of a big one - multiple plots
  theme() #optional: function that controls the looks of the entire visualization - font, color, etc
...
#ggplot data is globally declared and inherited down
#in this example, geom_point has a local declaration
```
...
```{r}
ggplot(data = mpg, mapping = aes(x = cty, y = hwy)) + #global declaration from ggplot
  geom_point() +
  geom_smooth()
```
...
```{r}
ggplot() +
  geom_point(data = mpg, mapping = aes(x = cty, y = hwy)) + #locally declared aes
  geom_smooth()
```
