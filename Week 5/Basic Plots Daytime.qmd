...
ggplot(mpg, aes(x = hwy)) +
  geom_histogram()
#in a histogram since it is a uni-variate distribution, we do not have a y variable
```
...
```{r}
ggplot(mpg, aes(x = hwy)) +
  geom_histogram(bins = 15, color = "hotpink", fill = 'limegreen', alpha =.4)
```
...
```{r}
ggplot(mpg, aes(x = factor(cyl),y = hwy)) + # factor -> converts continuous variable into discrete
...
```{r}
mpg %>% 
  filter(cyl != 5) %>% #filter out 5 cylinder cars
...
mpg %>% 
  ...
  geom_col(fill = 'black') # lets us add y variable
```
...
mpg %>% 
  group_by(class) %>% 
  summarize(count = n()) %>% 
  ggplot(aes(x = reorder(class, -count), y = count)) + # + / - count allows us to flip the direction
  geom_col()
```
...
mpg %>% 
  ...
  ggplot(aes(x = reorder(class, count), y = count)) +
  geom_col() +
  geom_text(aes(label = count, y = count - 2), color = "white") + #important
  labs(x = "Vehicle Class", y = "Count",
